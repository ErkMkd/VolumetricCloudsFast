...
def light_to_dict(light:hg.Node):
	name=light.GetName()
	lt=light.GetLight()
	light_dict={
				name+".diffuse" : color_to_list(lt.GetDiffuseColor()),
				name+".shadowColor" : color_to_list(lt.GetShadowColor()),
				name+".model":lt.GetModel(),
				name+".shadow":lt.GetShadow(),
				name+".shadowRange":lt.GetShadowRange(),
				name+".range":lt.GetRange(),
				name+".diffuseIntensity":lt.GetDiffuseIntensity()
				}
	return light_dict

def dict_to_light(light:hg.Node,script:dict):
	name = light.GetName()
	lt = light.GetLight()
	lt.SetDiffuseColor(list_to_color(script[name+".diffuse"]))
	lt.SetShadowColor(list_to_color(script[name+".shadowColor"]))
	lt.SetModel(script[name+".model"])
	lt.SetShadow(script[name+".shadow"])
	lt.SetShadowRange(script[name+".shadowRange"])
	lt.SetRange(script[name+".range"])
	lt.SetDiffuseIntensity(script[name+".diffuseIntensity"])
...
def load_json_camera(file_name):
	json_script = hg.GetFilesystem().FileToString(file_name)
	if json_script != "":
		script_parameters = json.loads(json_script)
		pos = list_to_vec3(script_parameters["position"])
		rot = list_to_vec3_radians(script_parameters["rotation"])
		zoom_factor = script_parameters["zoom_factor"]
		return pos,rot,zoom_factor
	return None,None
...
def save_json_camera(camera:hg.Node,output_filename ):
	pos=camera.GetTransform().GetPosition()
	rot=camera.GetTransform().GetRotation()
	zm=camera.GetCamera().GetZoomFactor()
	script_parameters = {"position" : vec3_to_list(pos), "rotation" : vec3_to_list_degrees(rot), "zoom_factor" : zm }
	json_script = json.dumps(script_parameters, indent=4)
	return hg.GetFilesystem().StringToFile(output_filename, json_script)
