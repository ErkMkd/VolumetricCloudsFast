...
from math import radians, pi, sin, cos, floor
...
class CloudsLayer(ViewTrame):
	...
	def create_particles(plus, scene, file_name, num, name):
		...
		for i in range(num):
			...
			# scene.AddNode(node)
			particles.append([geo, hg.Matrix4(),geo.GetMaterial(0)])  # node.SetEnabled(False)
		return particles
	...
	def set_particle(self, position: hg.Vector2):
		...
		if self.particle_index[id] < self.num_particles[id]:
			...
			if c.x > self.alpha_threshold:
				...
				#	mat_rot = hg.Matrix3.Identity #(hg.Matrix3.LookAt((self.cam_pos - pos).Normalized()))

				#elif self.billboard_type == CloudsLayer.billboard3D:
				#	mat_rot = hg.Matrix3.Identity #hg.Matrix3.FromEuler(self.rot_hash * c.x + hg.Vector3(0.1, 0.2, 0.3) * (1 - c.x) * self.t)
...
class Clouds:
	...
	def update(self, t, delta_t, scene, mat, resolution):
		self.t = t
		self.cam_pos = mat.GetTranslation() #camera.GetTransform().GetPosition()
